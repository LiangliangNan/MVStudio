...
PaintCanvas::PaintCanvas(QWidget *parent /* = nullptr*/)
        : QOpenGLWidget(parent), func_(nullptr), camera_(nullptr), pressed_button_(Qt::NoButton),
          mouse_pressed_pos_(0, 0), mouse_previous_pos_(0, 0), show_pivot_point_(false), drawable_axes_(nullptr),
          model_idx_(-1), point_set_(nullptr), project_(nullptr) {
...
void PaintCanvas::initializeGL() {
    ...
    if (!func_->hasOpenGLFeature(QOpenGLFunctions::Multisample))
        throw std::runtime_error("Multisample not supported on this machine!!! PaintCanvas may not run properly");
    if (!func_->hasOpenGLFeature(QOpenGLFunctions::Framebuffers))
        throw std::runtime_error(
                "Framebuffer Object is not supported on this machine!!! PaintCanvas may not run properly");
    ...
    if (major * 10 + minor < 32) {
        throw std::runtime_error("PaintCanvas requires at least OpenGL 3.2");
    }
...
void PaintCanvas::keyPressEvent(QKeyEvent *e) {
    if (e->key() == Qt::Key_F1 && e->modifiers() == Qt::NoModifier)
        ...
        if (camera_->type() == easy3d::Camera::PERSPECTIVE)
            camera_->setType(easy3d::Camera::ORTHOGRAPHIC);
        else
            camera_->setType(easy3d::Camera::PERSPECTIVE);
...
void PaintCanvas::setProjectionMode(bool b) {
    if (b)
        camera()->setType(easy3d::Camera::PERSPECTIVE);
    else
...
void PaintCanvas::showPointAsSphere(bool b) {
    for (auto m : models()) {
        for (auto d : m->points_drawables())
            d->set_impostor_type(b ? PointsDrawable::SPHERE : PointsDrawable::PLAIN);
    }
    update();
}
...
void PaintCanvas::increasePointSize() {
    for (auto m : models()) {
        for (auto d : m->points_drawables()) {
            float size = d->point_size();
            d->set_point_size(size + 1);
        }
    }
    update();
}
