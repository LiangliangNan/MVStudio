...
public:
	...
	Quaternion(const vec3d& from, const vec3d& to);
	...
	void set_from_rotated_basis(const vec3d& X, const vec3d& Y, const vec3d& Z);
	...
	void get_axis_angle(vec3d& axis, double& angle) const;
	...
	friend vec3d operator*(const Quaternion& q, const vec3d& v) { return q.rotate(v); }
