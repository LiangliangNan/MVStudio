...
{
	Image_var img = ImageIO::read(imagepath);
	if (!img) {
		std::cerr << "Unable to open image \'" << imagepath << "\'" << std::endl;
		return 0;
	}

	// in OpenGL, the image has to be v-flipped.
	flip_image(img);
	int glformat = GL_RGB;
	if (img->color_encoding() == Image::RGB)
		glformat = GL_RGB;
	else if (img->color_encoding() == Image::RGBA)
		glformat = GL_RGBA;
	else {
		std::cout << "Unsupported image color encoding" << std::endl;
		return 0;
	}
	...
	if (SetImageData(w, h, img->base_mem(), glformat, GL_UNSIGNED_BYTE) == 0)
		return 0;
