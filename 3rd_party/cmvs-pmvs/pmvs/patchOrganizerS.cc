...
void CpatchOrganizerS::writePatches2(PointSet* pset, const std::string prefix, bool bExportPLY, bool bExportPatch, bool bExportPSet) {
	...
	{
		char buffer[1024];
		sprintf(buffer, "%s/dense.patch", prefix.c_str());
		ofstream ofstr;
		ofstr.open(buffer);
		ofstr << "PATCHES" << endl
	...
	{
		char buffer[1024];
		sprintf(buffer, "%s/dense.pset", prefix.c_str());
		ofstream ofstr;
		ofstr.open(buffer);
		for (int p = 0; p < (int)m_ppatches.size(); ++p)
...
void CpatchOrganizerS::readPatches(void) {
	...
	for (int i = 0; i < m_fm.m_tnum; ++i) {
		...
		char buffer[1024];
		sprintf(buffer, "%s/models/%08d.patc%d",
	...
	for (int i = m_fm.m_tnum; i < m_fm.m_num; ++i) {
		const int image = m_fm.m_images[i];
		char buffer[1024];
		sprintf(buffer, "%s/models/%08d.patc%d", m_fm.m_prefix.c_str(), image, m_fm.m_level);
		ifstream ifstr;
		ifstr.open(buffer);
		if (!ifstr.is_open())
			continue;
