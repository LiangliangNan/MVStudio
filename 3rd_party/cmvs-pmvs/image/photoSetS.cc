...
void CphotoSetS::init(const std::vector<int>& images, const std::string prefix,
  ...
  for (int index = 0; index < m_num; ++index) {
    ...
    sprintf(test0, "%s/visualize/%08d.ppm", prefix.c_str(), image);
    sprintf(test1, "%s/visualize/%08d.jpg", prefix.c_str(), image);
    sprintf(test2, "%s/visualize/%08d.png", prefix.c_str(), image);
    sprintf(test3, "%s/visualize/%08d.tiff", prefix.c_str(), image);
    if (ifstream(test0) || ifstream(test1)
      ...
      // Set name
      sprintf(name, "%s/visualize/%08d", prefix.c_str(), image);
      sprintf(mname, "%s/masks/%08d", prefix.c_str(), image);
      sprintf(ename, "%s/edges/%08d", prefix.c_str(), image);
      sprintf(cname, "%s/txt/%08d.txt", prefix.c_str(), image);
    ...
    // try 4 digits
      ...
      // Set name
      sprintf(name, "%s/visualize/%04d", prefix.c_str(), image);
      sprintf(mname, "%s/masks/%04d", prefix.c_str(), image);
      sprintf(ename, "%s/edges/%04d", prefix.c_str(), image);
      sprintf(cname, "%s/txt/%04d.txt", prefix.c_str(), image);
