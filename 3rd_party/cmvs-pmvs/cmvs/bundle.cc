...
void Cbundle::prep(const std::string prefix, const int imageThreshold,
  ...
  char buffer[1024];
  sprintf(buffer, "%s/bundle.rd.out", prefix.c_str());
...
void Cbundle::writeVis(void) {
  ofstream ofstr;
  char buffer[1024];
  sprintf(buffer, "%s/vis.dat", m_prefix.c_str());
...
void Cbundle::writeCameraCenters(void) {
  for (int i = 0; i < (int)m_timages.size(); ++i) {
    char buffer[1024];
    sprintf(buffer, "%s/centers-%04d.ply", m_prefix.c_str(), i);
  ...
  {
    char buffer[1024];
    sprintf(buffer, "%s/centers-all.ply", m_prefix.c_str());
    ofstream ofstr;
    ofstr.open(buffer);
...
void Cbundle::writeGroups(void) {
  char buffer[1024];
  sprintf(buffer, "%s/ske.dat", m_prefix.c_str());
  ofstream ofstr;
  ofstr.open(buffer);
  ofstr << "SKE" << endl
